...
showLabelledTree
  :: Show a => Int -> a -> (a -> a -> a) -> LabelledTree a -> String
...
highlight globalVars = do
  gv@GlobalVars { graph = g, getRules = rs, selectedRule = r, highlighted = h, canvas = c } <-
    readIORef globalVars
...
layoutLoop globalVars = do
  gv@GlobalVars { graph = g, paused = p, layoutStep = l, canvas = c } <-
    readIORef globalVars
  unless p $ do
...
--	applyLeafRules' :: ([Match], Graph n) -> (Int, Rule n) -> (([Match], Graph n), (Int, Rule n))
  applyLeafRules' (matches, g) (n, r) =
    let
      ms = runPattern r' g
      r' =
        restrictOverlap (\past future -> future `elem` matches) (restriction r)
    in
      if null ms
        then ((matches, g), (n, r))
        else
          let (match, rewrite) = head ms
              g'               = execGraph rewrite g
          in  applyLeafRules' (filter (not . any (`elem` match)) matches, g')
                              (n + 1, r)
