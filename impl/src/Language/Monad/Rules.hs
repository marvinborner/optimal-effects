...
commute :: (View [Port] n, View NodeMS n) => Rule n
commute = do
  n1 :-: n2 <- activePair
...
annihilate :: (View [Port] n, View NodeMS n) => Rule n
annihilate = do
  n1 :-: n2 <- activePair
  require (n1 == n2)
...
eliminateDuplicator :: (View [Port] n, View NodeMS n) => Rule n
...
eraser :: (View [Port] n, View NodeMS n) => Rule n
eraser = do
  rewrite  <- commute
  Eraser{} <- liftReader . inspectNode =<< previous
  return rewrite

duplicate :: (View [Port] n, View NodeMS n) => Rule n
duplicate = do
  rewrite      <- commute
  Duplicator{} <- liftReader . inspectNode =<< previous
  return rewrite

bindExecBind :: (View [Port] n, View NodeMS n) => Rule n
bindExecBind = do
...
-- TODO: verify
bindExecUnit :: (View [Port] n, View NodeMS n) => Rule n
bindExecUnit = do
  BindN { inp = p, next = n, var = v, exec = True } :-: UnitN { out = o } <-
    activePair
  replace $ do
    byWire p n
    byWire o v
...
-- initializeUnitExec :: (View [Port] n, View NodeMS n) => Rule n
...
-- initializeBindExec :: (View [Port] n, View NodeMS n) => Rule n
...
-- applyUnitExec' :: Rule (Layout.Wrapper NodeMS)
-- applyUnitExec' = do
--   UnitN { inp = p, exec = True } :-: Actor { function = f, args = a } <-
--     activePair
--   replace $ f a p

-- applyBindExec' :: Rule (Layout.Wrapper NodeMS)
-- applyBindExec' = do
...
-- instance EffectApplicable (Layout.Wrapper NodeMS) where
--   applyUnitEffect = applyUnitExec'
--   applyBindEffect = applyBindExec'

-- applyUnitExec :: (EffectApplicable n, View [Port] n, View NodeMS n) => Rule n
-- applyUnitExec = do
--   eff <- applyUnitEffect
--   -- optional $ exhaustive compileShare -- TODO!
--   return eff

-- applyBindExec :: (EffectApplicable n, View [Port] n, View NodeMS n) => Rule n
-- applyBindExec = do
--   eff <- applyBindEffect
--   -- optional $ exhaustive compileShare -- TODO!
--   return eff
