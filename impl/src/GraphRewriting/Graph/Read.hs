{-# LANGUAGE FlexibleContexts, FlexibleInstances, TypeSynonymInstances, MultiParamTypeClasses, ScopedTypeVariables #-}
...
readOnly r = asks (runReader r)

existNode :: MonadReader (Graph n) m => Node -> m Bool
existNode (Node n) = asks (Map.member n . nodeMap)

readNode :: (MonadReader (Graph n) m, MonadFail m) => Node -> m n
readNode (Node n) =
  maybe (fail $ "readNode: node with ID " ⧺ show n ⧺ " does not exist") return
    .   readRef n
    =<< asks nodeMap

-- | a wrapper to 'inspect' the given node
inspectNode :: (View v n, MonadReader (Graph n) m, MonadFail m) => Node -> m v
inspectNode = fmap inspect . readNode

-- | a wrapper to 'examine' the given node
examineNode
  :: (View v n, MonadReader (Graph n) m, MonadFail m) => (v -> a) -> Node -> m a
examineNode f = fmap (examine f) . readNode

-- | all of the graph's nodes
readNodeList :: MonadReader (Graph n) m => m [Node]
readNodeList = fmap (map Node . Map.keys) (asks nodeMap)

-- | all of the graph's edges
readEdgeList :: MonadReader (Graph n) m => m [Edge]
readEdgeList = fmap (map Edge . Map.keys) (asks edgeMap)

-- | edges attached to the given node
attachedEdges
  :: (View [Port] n, MonadReader (Graph n) m, MonadFail m) => Node -> m [Edge]
attachedEdges = fmap nub . inspectNode

-- | non-empty set of nodes attached to the given edge
attachedNodes :: (MonadReader (Graph n) m, MonadFail m) => Edge -> m [Node]
attachedNodes = fmap (map Node . Set.elems) . readEdge

-- | amount of ports the given hyperedge is connected to
edgeCardinality
  :: (View [Port] n, MonadReader (Graph n) m, MonadFail m) => Edge -> m Int
edgeCardinality e =
  fmap (length . filter (e ≡) . concat) (mapM inspectNode =<< attachedNodes e)

-- | list of nodes that are connected to the given node, not including the node itself
neighbours
  :: (View [Port] n, MonadReader (Graph n) m, MonadFail m) => Node -> m [Node]
neighbours n = do
...
adverseNodes
  :: (MonadReader (Graph n) m, MonadFail m) => Node -> Port -> m [Node]
adverseNodes (Node n) p =
  fmap (map Node . Set.elems . Set.delete n) (readEdge p)
...
connected
  ...
  -> Node
  -> m Bool
connected n1 n2 = fmap (n2 ∈) (relatives n2)

-- | whether the given ports features a dangling edge
dangling
  :: (View [Port] n, MonadReader (Graph n) m, MonadFail m) => Port -> m Bool
dangling = fmap (≡ 1) . edgeCardinality
